...
const AppMenu = ({
  ...
  const activeMenu = useCallback(() => {
    ...
    if (matchingMenuItem) {
      ...
      setTimeout(() => {
        // Try to find the link - React Router uses data attributes or href might be different
        // Try exact match first, then try prefix match
        let activatedItem = document.querySelector(`#leftside-menu-container .simplebar-content a[href="${trimmedURL}"]`);
        if (!activatedItem) {
          // Try to find link that starts with the menu item URL
          const menuItemUrl = matchingMenuItem.url;
          if (menuItemUrl) {
            const allLinks = document.querySelectorAll(`#leftside-menu-container .simplebar-content a[href^="${menuItemUrl}"]`);
            if (allLinks.length > 0) {
              activatedItem = allLinks[0];
            }
          }
        }
        if (activatedItem) {
