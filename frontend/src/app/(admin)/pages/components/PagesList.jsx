...
const PagesList = () => {
  ...
  const {
    ...
    refetch,
    clearFilters,
  } = useFilteredPaginatedData('/pages/', '/page-categories/?is_active=true', {
    defaultPageSize: 10,
    debounceMs: 500,
  ...
  const handleDelete = async (uuid) => {
    ...
    setDeletingId(uuid);
    try {
      await httpClient.delete(`/pages/${uuid}/`);
      showNotification({ message: 'Page deleted successfully.', variant: 'success' });
      refetch(pagination.currentPage);
    } catch (error) {
      showNotification({ message: 'Failed to delete page.', variant: 'danger' });
    } finally {
      setDeletingId(null);
    }
  };
    ...
    return (
      <div className="d-flex justify-content-between align-items-center mt-3">
        <div className="text-muted">
          Showing {(pagination.currentPage - 1) * pagination.pageSize + 1} to {Math.min(pagination.currentPage * pagination.pageSize, pagination.total)} of {pagination.total} pages
        </div>
        <Pagination>
  ...
  return (
    <ComponentContainerCard title="Pages Management" description="Manage static content pages.">
      <Row className="mb-3">
        <Col md={6}>
          <Button variant="primary" onClick={() => navigate('/pages/create')}>
          ...
          <Row className="g-3">
            <Col md={4}>
              <InputGroup>
                ...
                <FormControl placeholder="Search pages..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
              </InputGroup>
            </Col>
            <Col md={3}>
                  ...
                  <tr key={item.id}>
                    ...
                    <td>
                      <Dropdown>
                        ...
                        <Dropdown.Menu>
                          <Dropdown.Item onClick={() => navigate(`/pages/${item.uuid}/edit`)}>
