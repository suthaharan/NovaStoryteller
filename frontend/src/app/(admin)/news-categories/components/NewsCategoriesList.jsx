...
const NewsCategoriesList = () => {
  ...
  const fetchCategories = useCallback(async (page = 1, search = '') => {
    ...
    try {
      ...
      const response = await httpClient.get(`/news-categories/?${params.toString()}`);
      ...
      showNotification({ message: 'Failed to load news categories.', variant: 'danger' });
    } finally {
      setLoading(false);
  ...
  const handleDelete = async (uuid, categoryName) => {
    ...
    try {
      await httpClient.delete(`/news-categories/${uuid}/`);
  ...
  const handleToggleActive = async (uuid, currentStatus) => {
    try {
      await httpClient.patch(`/news-categories/${uuid}/`, {
  ...
  return (
    <ComponentContainerCard title="News Categories Management" description="Manage news categories.">
      <Row className="mb-3">
        <Col md={6}>
          <Button variant="primary" onClick={() => navigate('/news-categories/create')}>
              ...
              <tbody>
                {categories.map((item) => (
                  <tr key={item.id}>
                    ...
                    <td><Badge bg="info">{item.news_count || 0}</Badge></td>
                    <td>
                    ...
                    <td>
                      <Dropdown>
                        ...
                        <Dropdown.Menu>
                          <Dropdown.Item onClick={() => navigate(`/news-categories/${item.uuid}/edit`)}>
