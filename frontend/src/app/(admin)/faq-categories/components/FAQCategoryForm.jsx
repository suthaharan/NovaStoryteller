...
const FAQCategoryForm = () => {
  ...
  useEffect(() => {
    if (isEdit) {
      const fetchCategory = async () => {
        setLoading(true);
        try {
          const response = await httpClient.get(`/faq-categories/${uuid}/`);
  ...
  const onSubmit = handleSubmit(async (values) => {
    ...
    try {
      if (isEdit) {
        await httpClient.put(`/faq-categories/${uuid}/`, values);
        showNotification({ message: 'Category updated successfully.', variant: 'success' });
      } else {
        await httpClient.post('/faq-categories/', values);
  ...
  if (loading && isEdit) {
    return (
      <ComponentContainerCard title={isEdit ? 'Edit FAQ Category' : 'Create FAQ Category'}>
  ...
  return (
    <ComponentContainerCard 
      title={isEdit ? 'Edit FAQ Category' : 'Create FAQ Category'}
      description={isEdit ? 'Update FAQ category information.' : 'Create a new FAQ category.'}
    >
      <Card>
        <CardBody>
          <form onSubmit={onSubmit}>
            ...
            <Row className="mt-4">
              <Col>
                ...
                <Button type="button" variant="outline-secondary" className="ms-2" onClick={() => navigate('/faq-categories')}>
                  Cancel
                </Button>
              </Col>
            </Row>
