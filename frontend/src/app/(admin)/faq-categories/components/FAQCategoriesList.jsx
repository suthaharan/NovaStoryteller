...
const FAQCategoriesList = () => {
  ...
  const fetchCategories = useCallback(async (page = 1, search = '') => {
    ...
    try {
      ...
      const response = await httpClient.get(`/faq-categories/?${params.toString()}`);
      ...
      showNotification({ message: 'Failed to load FAQ categories.', variant: 'danger' });
    } finally {
      setLoading(false);
  ...
  const handleDelete = async (uuid, categoryName) => {
    ...
    try {
      await httpClient.delete(`/faq-categories/${uuid}/`);
  ...
  const handleToggleActive = async (uuid, currentStatus) => {
    try {
      await httpClient.patch(`/faq-categories/${uuid}/`, {
  ...
  return (
    <ComponentContainerCard title="FAQ Categories Management" description="Manage FAQ categories.">
      <Row className="mb-3">
        <Col md={6}>
          <Button variant="primary" onClick={() => navigate('/faq-categories/create')}>
              ...
              <tbody>
                {categories.map((item) => (
                  <tr key={item.id}>
                    ...
                    <td><Badge bg="info">{item.faq_count || 0}</Badge></td>
                    <td>
                    ...
                    <td>
                      <Dropdown>
                        ...
                        <Dropdown.Menu>
                          <Dropdown.Item onClick={() => navigate(`/faq-categories/${item.uuid}/edit`)}>
